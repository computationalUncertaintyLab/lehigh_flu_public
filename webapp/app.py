...
def create_forecast_plot(samples, data, show_historical=True, show_predictions=True):
    ...
    if show_historical:
        ...
        for i in range(nseasons - 1):  # Exclude current season
            ...
            # Plot median for historical seasons
            ax.plot(med, color=colors[i], alpha=0.7, linewidth=2, 
            ...
            if np.any(observed_mask):
                ax.scatter(np.arange(33)[observed_mask], cases[i][observed_mask], 
                          color=colors[i], s=20, alpha=0.8, zorder=5)
    
    # Plot current season observed data
    ...
    if np.any(observed_mask):
        ax.scatter(np.arange(33)[observed_mask], cases[current_season_idx][observed_mask], 
                  color='black', s=30, alpha=0.9, zorder=6, label='Current season (observed)')
...
def main():
    ...
    if forecast_data is None or historical_data is None:
        st.stop()
